...
M.cancel = Promise.async(function()
  if state.active_session then
    if state.jobs.is_running() then
      vim.g.pi_abort_count = (vim.g.pi_abort_count or 0) + 1
    end
    ...
    if vim.g.pi_abort_count >= 3 then
      vim.notify('Re-starting Pi server', vim.log.levels.WARN)
      vim.g.pi_abort_count = 0
      if state.pi_server then
        state.pi_server:shutdown():await()
      end

      state.jobs.clear_server()
      state.jobs.set_server(server_job.ensure_server():await() --[[@as PiServer]])
    end
  end

  if state.ui.is_visible() then
    require('pi.ui.footer').clear()
    input_window.set_content('')
    require('pi.history').index = nil
    ui.focus_input()
  end
end)

M.pi_ok = Promise.async(function()
  if config.backend == 'pi' then
    ...
    state.jobs.set_pi_cli_version(promise)
    return true
  end

  if vim.fn.executable(config.pi_executable) == 0 then
    vim.notify(
      'pi command not found - please install and configure pi before using this plugin',
      vim.log.levels.ERROR
    )
    return false
  end

  if not state.pi_cli_version then
    local promise = Promise.system({ config.pi_executable, '--version' }):and_then(function(result)
  ...
  local current_version = state.pi_cli_version:await()

  if not current_version or current_version == '' then
    vim.notify(string.format('Unable to detect pi CLI version. Requires >= %s', required), vim.log.levels.ERROR)
    return false
  end

  if not util.is_version_greater_or_equal(current_version, required) then
    vim.notify(
      string.format('Unsupported pi CLI version: %s. Requires >= %s', current_version, required),
      vim.log.levels.ERROR
    )
    return false
  end
...
M._on_user_message_count_change = Promise.async(function(_, new, old)
  require('pi.ui.renderer.flush').flush_pending_on_data_rendered()
