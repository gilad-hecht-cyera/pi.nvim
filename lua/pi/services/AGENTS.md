...
The following entry files still directly require `pi.session`/`pi.api` and should be removed by routing through services APIs.

- [ ] `lua/pi/quick_chat.lua` -> `pi.session`
- [ ] `lua/pi/ui/renderer.lua` -> `pi.session`, `pi.api`
- [ ] `lua/pi/ui/debug_helper.lua` -> `pi.session`
- [ ] `lua/pi/ui/permission_window.lua` -> `pi.api`
- [ ] `lua/pi/ui/contextual_actions.lua` -> `pi.api`
- [ ] `lua/pi/ui/session_picker.lua` -> `pi.api`
- [ ] `lua/pi/ui/timeline_picker.lua` -> `pi.api`
- [ ] `lua/pi/ui/ui.lua` -> `pi.api`
- [ ] `lua/pi/commands/handlers/diff.lua` -> `pi.session`
- [ ] `lua/pi/commands/handlers/session.lua` -> `pi.session`

Sync rule:
- keep this list aligned with `lua/pi/commands/handlers/AGENTS.md`
- remove an item only after code + tests pass

## Hard invariants

- Entry modules must prefer `services/*` over adding new direct `pi.session` / `pi.api` requires
...
- New direct entry-layer `require('pi.session')` or `require('pi.api')` is introduced without explicit exception approval
...
- `./run_tests.sh`
- `rg -n "require\(['\"]pi\.(session|api)['\"]\)" lua/pi/ui lua/pi/commands/handlers lua/pi/quick_chat.lua`
- `! grep -n "vim\.api\|vim\.fn\|vim\.notify" lua/pi/services/messaging.lua`
- `! grep -n "vim\.api\|vim\.fn\|vim\.notify" lua/pi/services/agent_model.lua`
