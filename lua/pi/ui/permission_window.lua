...
---@param permission PiPermission|nil
...
function M.remove_permission(permission_id)
  ...
  require('pi.ui.renderer.events').render_permissions_display()
end

---Get currently selected permission (always the first one now)
---@return PiPermission|nil
...
function M.format_display(output)
  ...
  local formatter_utils = require('pi.ui.formatter.utils')
  ...
  M._dialog:format_dialog(output, {
    ...
    title_hl = 'PiPermissionTitle',
    border_hl = 'PiPermissionBorder',
    content = content,
    render_content = render_content,
    options = options,
    unfocused_message = 'Focus Pi window to respond to permission',
...
function M._setup_dialog()
  ...
  local function check_focused()
    ...
    return ui.is_pi_focused() and #M._permission_queue > 0
  end
  ...
  local function on_select(index)
    ...
    vim.schedule(function()
      ...
      if action == 'deny' then
        ...
        local part_data = require('pi.ui.renderer.ctx').render_state:get_part('permission-display-part')
        ...
        if output_win and vim.api.nvim_win_is_valid(output_win) then
          ...
          interaction.feedback = require('pi.ui.inline_input').open({
            win = output_win,
            row = row,
            col = col,
  ...
  M._dialog = Dialog.new({
    ...
    on_dismiss = function()
      ...
      if interaction.deny_armed then
        clear_deny_timer(interaction)
        M._processing = true
        require('pi.api').permission_deny(current_permission, nil)
    ...
    namespace_prefix = 'pi_permission',
