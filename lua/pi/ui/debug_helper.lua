...
function M.open_json_file(data)
  ...
  if state.last_code_win_before_pi then
    vim.api.nvim_set_current_win(state.last_code_win_before_pi --[[@as integer]])
  end
...
function M.debug_output()
  local session_formatter = require('pi.ui.formatter')
