...
local M = {}
M.namespace = vim.api.nvim_create_namespace('pi_output')
M.debug_namespace = vim.api.nvim_create_namespace('pi_output_debug')
M.markdown_namespace = vim.api.nvim_create_namespace('pi_output_markdown')
...
function M.setup(windows)
  ...
  window_options.set_window_option('foldtext', 'v:lua.pi_fold_text()', windows.output_win)
...
function M.setup_autocmds(windows, group)
  ...
  vim.api.nvim_create_autocmd('TabEnter', {
    ...
    callback = function()
      if state.ui.is_window_in_current_tab(windows.output_win) then
        require('pi.ui.renderer.flush').resume_deferred_rendering()
      end
    end,
  })
  ...
  debounced_load_more_at_top = require('pi.util').debounce(function()
    local renderer = require('pi.ui.renderer')
    ...
    if top_line then
      for _, msg in ipairs(state.messages or {}) do
        ...
        if not msg_id:match('^__pi_') then
