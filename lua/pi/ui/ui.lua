...
---@param windows PiWindowState
---@return PiHiddenBuffers
local function capture_hidden_snapshot(windows)
  local current_win = vim.api.nvim_get_current_win()
  local focused = state.last_focused_pi_window or 'input'
...
---Clear Pi-specific autocmds and shared UI state before closing windows.
local function prepare_window_close()
  if M.is_pi_focused() then
  ...
  pcall(vim.api.nvim_del_augroup_by_name, 'PiResize')
  pcall(vim.api.nvim_del_augroup_by_name, 'PiWindows')
  pcall(vim.api.nvim_del_augroup_by_name, 'PiFooterResize')
...
function M.restore_hidden_windows()
  ...
  local autocmds = require('pi.ui.autocmds')
  ...
  require('pi.ui.contextual_actions').setup_contextual_actions(windows)
...
---Return focus to the window that was active before opening Pi.
function M.return_to_last_code_win()
  local last_win = state.last_code_win_before_pi
...
function M.create_windows()
  if config.ui.enable_treesitter_markdown then
    ...
    if ok and treesitter and treesitter.language and treesitter.language.register then
      treesitter.language.register('markdown', 'pi_output')
      treesitter.language.register('markdown', 'pi')
    end
  end

  local autocmds = require('pi.ui.autocmds')

  if not require('pi.ui.ui').is_pi_focused() then
  ...
  local filetype = config.ui.output.filetype or 'pi_output'
...
---@return boolean
function M.is_pi_focused()
  if not state.windows then
    return false
  end
  local current_win = vim.api.nvim_get_current_win()
  return M.is_pi_window(current_win)
end
...
---@param opts? {force_scroll?: boolean}
---@return Promise<PiMessage[]> | PiMessage[] | nil
...
function M.swap_position()
  ...
  vim.schedule(function()
    require('pi.api').toggle(state.active_session == nil)
  end)
end

---Toggle the current Pi window width between normal and zoomed.
