...
local namespace = vim.api.nvim_create_namespace('pi_contextual_actions')
local augroup = vim.api.nvim_create_augroup('PiContextualActions', { clear = true })
local lifecycles = {}
...
function M.show_contextual_actions_menu(buf, actions)
  ...
  for _, action in ipairs(actions) do
    vim.api.nvim_buf_set_extmark(buf, namespace, action.display_line, 0, {
      virt_text = { { '⋮ ' .. action.text .. ' ', 'PiContextualActions' } },
    ...
    if action.key and lifecycle.saved_mappings[action.key] == nil then
      ...
      vim.keymap.set('n', action.key, function()
        ...
        if action.type and action.args then
          require('pi.api')[action.type](unpack(action.args))
        end
...
local function refresh_contextual_actions(buf)
  ...
  M.show_contextual_actions_menu(buf, require('pi.ui.renderer').get_actions_for_line(line))
end
