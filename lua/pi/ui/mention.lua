...
local mentions_namespace = vim.api.nvim_create_namespace('PiMentions')
...
---@param mentions PiMessagePartSourceText[] Mention data with character offsets
  ...
  for _, mention in ipairs(mentions) do
    ...
    for i, line in ipairs(vim.split(text, '\n')) do
      ...
      if char_start == 0 and string.sub(text, 0, 1) ~= '@' then
        -- Work around Pi bug? where mentions sometimes have a 0 start
...
function M.mention(get_name)
  local windows = require('pi.state').windows
...
function M.restore_mentions(buf)
  ...
  M.highlight_all_mentions(buf, function(name)
    local agents = require('pi.config_file').get_subagents()

    if vim.tbl_contains(agents, name) then
      require('pi.context').add_subagent(name)
      return
    end

    if vim.fn.filereadable(name) == 1 then
      require('pi.context').add_file(name)
      return
    end
    if name:find('^pasted_image_') then
      local restored_path = require('pi.image_handler').restore_img_path(name)
      if restored_path then
        require('pi.context').add_file(restored_path)
      end
    end
  end)
end
