local context_module = require('pi.context')
...
function M._format_reasoning(output, part)
  ...
  if end_line - start_line > 1 then
    M.add_vertical_border(output, start_line, end_line, 'PiToolBorder', -1, 'PiReasoningText')
  else
    output:add_extmark(start_line - 1, {
      line_hl_group = 'PiReasoningText',
    } --[[@as OutputExtmark]])
  end
end

---Format the revert callout with statistics
---@param session_data PiMessage[] All messages in the session
    ...
    for file, fstats in pairs(stats.files) do
      ...
      if #file_diff > 0 then
        ...
        for _, diff in ipairs(file_diff) do
          local hl_group = diff:sub(1, 1) == '+' and 'PiDiffAddText' or 'PiDiffDeleteText'
...
---@param message PiMessage
---@param previous_message? PiMessage
---@return Output
function M.format_message_header(message, previous_message)
  local output = Output.new()

  if message.info and message.info.id == '__pi_revert_message__' then
    output:add_lines(M.separator)
    return output
  end

  if message.info and message.info.id == '__pi_hidden_messages_notice__' then
    return output
  end
  ...
  local role_hl = 'PiMessageRole' .. role:sub(1, 1):upper() .. role:sub(2)
  ...
  if time and (role ~= 'assistant' or header_style ~= 'hidden') then
    output:add_extmark(output:get_line_count() - 1, {
      virt_text = { { (same_mode_as_previous and '' or ' ') .. util.format_time(time), 'PiHint' } },
...
---@param output Output Output object to write to
---@param text string
---@param message? PiMessage Optional message object to extract mentions from
function M._format_user_prompt(output, text, message)
  ...
  M.add_vertical_border(output, start_line, end_line + end_line_extmark_offset, 'PiMessageRoleUser', -3)
end

---@param output Output
local function format_compaction_divider(output)
  output:add_line(compaction_divider_text)
  output:add_extmark(output:get_line_count() - 1, { line_hl_group = 'PiBorder', priority = 5000 })
end
...
function M._format_selection_context(output, part)
  ...
  M.add_vertical_border(output, start_line, end_line, 'PiMessageRoleUser', -3)
end
...
function M._format_diagnostics_context(output, part)
  ...
  local start_line = output:get_line_count()
  local diagnostics = json.content --[[@as PiDiagnostic[] ]]
...
function M.format_tool(output, part, context)
  ...
  local end_line = output:get_line_count()
  if end_line - start_line > 1 then
    M.add_vertical_border(output, start_line, end_line, 'PiToolBorder', -1)
  end
end
...
function M.add_vertical_border(output, start_line, end_line, hl_group, win_col, text_hl_group)
  local extmark_opts = {
    virt_text = { { require('pi.ui.icons').get('border'), hl_group } },
...
---@param part PiMessagePart The part to format
---@param message? PiMessage Optional message object to extract role and mentions from
    ...
    elseif part.type == 'file' then
      ...
      if file_line then
        ...
        if next_kind == 'text' or (previous_is_context and not next_kind) then
          M.add_vertical_border(output, file_line - 1, file_line, 'PiMessageRoleUser', -3)
        elseif next_kind == 'file' then
          M.add_vertical_border(output, file_line, file_line + 1, 'PiMessageRoleUser', -3)
        else
          M.add_vertical_border(output, file_line, file_line, 'PiMessageRoleUser', -3)
        end
        content_added = true
      end
    end
