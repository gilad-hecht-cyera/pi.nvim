...
local prompt_guard_indicator = require('pi.ui.prompt_guard_indicator')
...
local function create_winbar_segments()
  ...
  if context.is_context_enabled('current_file') and current_file then
    local highlight = 'PiContextCurrentFile'
    if ctx.current_file and ctx.current_file.sent_at then
      highlight = 'PiContextCurrentFileNotUpdated'
    end
    ...
    for _, type_name in pairs(severity_types) do
      ...
      if count and count > 0 then
        table.insert(segments, {
          ...
          highlight = 'PiContext' .. type_name:gsub('^%l', string.upper),
        })
      end
    end
  end
...
local function format_winbar_text(segments)
  ...
  for i, segment in ipairs(segments) do
    ...
    if i < #segments then
      table.insert(parts, ' %#PiContextBar#|%* ')
    end
  end
...
function M.setup()
  state.store.subscribe(
    { 'current_context_config', 'current_code_buf', 'is_pi_focused', 'context_updated_at', 'user_message_count' },
    function()
      M.render()
    end
  )
...
function M.render(windows)
  vim.schedule(function()
    ...
    winbar.update_highlights(win, 'PiContextBar')
  end)
end
