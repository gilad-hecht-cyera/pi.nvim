...
local Timer = require('pi.ui.timer')
local M = {}
...
M._animation = {
  ...
  fps = 10,
  extmark_id = nil,
  ns_id = vim.api.nvim_create_namespace('pi_loading_animation'),
...
function M.sync_from_server()
  ...
  api_client
    ...
    :catch(function(err)
      require('pi.log').debug('loading_animation.sync_from_server failed: %s', tostring(err))
    end)
end
