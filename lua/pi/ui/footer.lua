...
local function build_right_segments()
  ...
  if loading_animation.is_running() then
    ...
    table.insert(segments, { string.format('%s ', cancel_keymap), 'PiInputLegend' })
    table.insert(segments, { 'to cancel', 'PiHint' })
    table.insert(segments, { ' ' })
  end

  if not loading_animation.is_running() and state.current_model and config.ui.display_model then
    table.insert(segments, { state.current_model, 'PiHint' })
    if state.current_variant then
      table.insert(segments, { '·', 'PiHint' })
      table.insert(segments, { state.current_variant, 'PiVariant' })
    end
    table.insert(segments, { ' ' })
  end
...
function M.render()
  ...
  ---@cast state.windows PiWindowState
...
function M.setup(windows)
  ...
  vim.api.nvim_set_option_value('winhl', 'Normal:PiHint', { win = windows.footer_win })
  ...
  vim.api.nvim_create_autocmd({ 'VimResized', 'WinResized' }, {
    group = vim.api.nvim_create_augroup('PiFooterResize', { clear = true }),
    callback = function()
...
function M.create_buf()
  ...
  vim.api.nvim_set_option_value('filetype', 'pi_footer', { buf = footer_buf })
  return footer_buf
end
...
function M.set_content(lines, highlights)
  ...
  local ns_id = vim.api.nvim_create_namespace('pi_footer')
