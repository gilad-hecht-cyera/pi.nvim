...
local buffer = require('pi.ui.renderer.buffer')
local output_diff = require('pi.ui.renderer.output_diff')
...
local function warn_part_render_error_once(part_id, message_id, err)
  ...
  vim.schedule(function()
    ...
    notify('[pi.nvim] ' .. msg, vim.log.levels.WARN)
  end)
end
...
local function do_trigger_on_data_rendered()
  ...
  vim.b[output_buf].pi_markdown_namespace = output_window.markdown_namespace
...
M.trigger_on_data_rendered =
  require('pi.util').debounce(do_trigger_on_data_rendered, config.ui.output.rendering.markdown_debounce_ms or 250)
