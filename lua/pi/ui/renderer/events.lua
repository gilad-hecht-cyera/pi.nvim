...
local function scroll(force)
  require('pi.ui.renderer').scroll_to_bottom(force)
end
...
function M.render_permissions_display()
  ...
  M.on_message_updated(fake_message --[[@as PiMessage]])
...
function M.on_message_updated(message, revert_index)
  ...
  local msg = message --[[@as PiMessage]]
  ...
  if not revert_index and not ctx.bulk_mode and msg.info.id ~= '__pi_hidden_messages_notice__' then
    require('pi.ui.renderer').reconcile_rendered_message_limit()
  end
end
...
function M.on_message_removed(properties)
  ...
  if not ctx.bulk_mode and message_id ~= '__pi_hidden_messages_notice__' then
    require('pi.ui.renderer').reconcile_rendered_message_limit()
  end
end

---Handle message.part.updated — insert or replace a part in the buffer
---@param properties {part: PiMessagePart}
---@param revert_index? integer
...
function M.on_session_updated(properties)
  ...
  if revert_changed then
    local real_messages = vim.tbl_filter(function(msg)
      return not (msg.info and msg.info.id and msg.info.id:match('^__pi_'))
    end, state.messages or {})
    require('pi.ui.renderer')._render_full_session_data(real_messages)
  end
end
...
function M.on_session_compacted(properties)
  ...
  require('pi.ui.renderer').render_full_session()
end
...
---@param properties PiQuestionRequest
