...
local window_options = require('pi.ui.window_options')
...
function M.create_buf()
  ...
  vim.api.nvim_set_option_value('filetype', 'pi', { buf = input_buf })

  local buffixwin = require('pi.ui.buf_fix_win')
...
function M.handle_submit()
  ...
  require('pi.services.messaging').send_message(input_content)
  return true
end
...
M._prompt_add_to_context = function(cmd, output, exit_code)
  ...
  picker.select({ 'Yes', 'No' }, {
    ...
    output_window.clear()
    require('pi.ui.input_window').focus_input()
  end)
end
...
M._append_to_input = function(text)
  ...
  require('pi.ui.mention').highlight_all_mentions(state.windows.input_buf)
...
M._execute_slash_command = function(command)
  local slash_commands = require('pi.commands.slash').get_commands():await()
  local key = config.get_key_for_function('input_window', 'slash_commands') or '/'
...
function M.setup(windows)
  ...
  window_options.set_buffer_option('filetype', 'pi', windows.input_buf)
  ...
  require('pi.ui.context_bar').render(windows)
end
...
function M.refresh_placeholder(windows, input_lines)
  ...
  if #input_lines == 1 and input_lines[1] == '' then
    ...
    local virt_text = {
      { 'Type your prompt here... ', 'PiHint' },
      { '!', 'PiInputLegend' },
      { ' shell ', 'PiHint' },
    }
    if slash_key then
      table.insert(virt_text, { slash_key, 'PiInputLegend' })
      table.insert(virt_text, { ' commands ', 'PiHint' })
    end
    if mention_key then
      table.insert(virt_text, { mention_key, 'PiInputLegend' })
      table.insert(virt_text, { ' mentions ', 'PiHint' })
    end
    if mention_file_key then
      table.insert(virt_text, { mention_file_key, 'PiInputLegend' })
      table.insert(virt_text, { ' files ', 'PiHint' })
    end
    if context_key then
      table.insert(virt_text, { context_key, 'PiInputLegend' })
      table.insert(virt_text, { ' context', 'PiHint' })
    end
    table.insert(virt_text, { padding, 'PiHint' })
...
function M.recover_input(windows)
  ...
  require('pi.ui.mention').highlight_all_mentions(windows.input_buf)
  M.update_dimensions(windows)
end
...
function M.focus_input()
  ...
  if #lines == 1 and lines[1] == '' then
    require('pi.ui.input_window').refresh_placeholder(state.windows)
  else
    require('pi.ui.input_window').clear_placeholder(state.windows)
  end
end
...
function M.refill_prompt_from_message(message)
  ...
  require('pi.ui.mention').restore_mentions(state.windows.input_buf)
...
function M.setup_autocmds(windows, group)
  vim.api.nvim_create_autocmd('WinEnter', {
    ...
    callback = function()
      ...
      require('pi.ui.context_bar').render()
    end,
  })
...
function M._hide()
  ...
  if was_at_bottom then
    vim.schedule(function()
      require('pi.ui.renderer').scroll_to_bottom(true)
    end)
  end
end
