...
---@field namespace_prefix? string Prefix for vim.on_key namespace (default: 'pi_dialog')
...
function Dialog:format_legend(output, options)
  ...
  if ui.is_pi_focused() then
    ...
    local message = options.unfocused_message or 'Focus Pi window to interact'
...
function Dialog:format_options(output, options)
  ...
  for i, option in ipairs(options) do
    ...
    if is_cursor then
      output:add_extmark(extmark_idx, { line_hl_group = 'PiDialogOptionHover' } --[[@as OutputExtmark]])
      if not self._is_multiple then
        output:add_extmark(extmark_idx, {
          start_col = 2,
          virt_text = { { '› ', 'PiDialogOptionHover' } },
