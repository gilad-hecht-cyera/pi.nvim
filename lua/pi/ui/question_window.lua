...
---@param question_request PiQuestionRequest|nil
---@return boolean
...
---@param parts PiMessagePart[]|nil
---@param question_request PiQuestionRequest|nil
---@return PiMessagePart|nil
...
local function get_question_part(question_request)
  ...
  if question_request and question_request.sessionID and question_request.sessionID ~= '' then
    local render_state = require('pi.ui.renderer.ctx').render_state
...
local function render_question()
  require('pi.ui.renderer.events').render_question_display()
end

---@param question_request PiQuestionRequest
...
---@param question_info PiQuestionInfo
---@return integer|nil
...
local function open_inline_other_input(request_id, question_index, option_index, on_submit)
  ...
  local part_data = require('pi.ui.renderer.ctx').render_state:get_part('question-display-part')
  ...
  M._clear_inline_input()
  local handle
  handle = require('pi.ui.inline_input').open({
...
local function format_question_tabs(output)
  ...
  for i, question in ipairs(request.questions) do
    ...
    table.insert(segments, {
      start_col = start_col,
      end_col = #line,
      hl_group = is_active and 'PiQuestionTabActive'
        or (is_done and 'PiQuestionTabDone' or 'PiQuestionTabPending'),
    })
  end
...
function M.format_display(output)
  ...
  M._dialog:format_dialog(output, {
    ...
    title_hl = 'PiQuestionTitle',
    border_hl = 'PiQuestionBorder',
    content = vim.split(question_info.question, '\n'),
    options = options,
    unfocused_message = 'Focus Pi window to answer question',
  })
end
...
function M._setup_dialog()
  ...
  local function check_focused()
    ...
    return ui.is_pi_focused() and not M._answering and is_active_question(request_id, question_index)
  end
