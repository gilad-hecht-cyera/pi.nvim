...
local skill_source = {
  ...
  on_complete = function(item)
    ...
    vim.defer_fn(function()
      require('pi.services.session_runtime').open({ new_session = false, focus = 'output' }):and_then(function()
        return require('pi.services.messaging').send_message(item.data.content, {})
      end)
    end, 10)
