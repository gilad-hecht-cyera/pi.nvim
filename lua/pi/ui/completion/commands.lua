...
local get_available_commands = Promise.async(function()
  local commands = require('pi.commands.slash').get_commands():await()
  ...
  for key, cmd_info in ipairs(commands) do
    table.insert(results, {
      ...
      documentation = 'Pi command: ' .. cmd_info.slash_cmd,
      command_key = key,
      args = cmd_info.args,
      fn = cmd_info.fn,
    })
...
local command_source = {
  name = 'commands',
  priority = 1,
  custom_kind = custom_kind.register('commands', require('pi.ui.icons').get('command')),
  ...
  on_complete = function(item)
    if item.kind == 'commands' then
      if item.data.fn then
        ...
        require('pi.ui.input_window').set_content('')
      else
