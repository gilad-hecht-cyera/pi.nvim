...
local function create_context_item(name, type, available, documentation, icon, additional_data, priority)
  ...
  return {
    ...
    kind_hl = not icon and available and 'PiContextSwitchOn' or nil,
    detail = name,
...
---@param cursor_data? PiContextCursorData
---@return string
...
local function remove_inserted_text(item)
  local input_win = require('pi.ui.input_window')
...
local context_source = {
  ...
  on_complete = function(item)
    ...
    elseif type == 'selection_item' then
      context.remove_selection(item.data.additional_data --[[@as PiContextSelection]])
    end
