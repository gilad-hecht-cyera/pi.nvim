...
local reference_facts = require('pi.ui.reference_facts')
local Promise = require('pi.promise')
local ctx = require('pi.ui.renderer.ctx')
local events = require('pi.ui.renderer.events')
local event_scope = require('pi.ui.event_scope')
local flush = require('pi.ui.renderer.flush')
local scroll = require('pi.ui.renderer.scroll')

local M = {}
local HIDDEN_MESSAGES_NOTICE_MESSAGE_ID = '__pi_hidden_messages_notice__'
local HIDDEN_MESSAGES_NOTICE_PART_ID = '__pi_hidden_messages_notice_part__'
...
local function is_renderer_synthetic_message(message)
  ...
  return message_id == '__pi_revert_message__' or message_id == HIDDEN_MESSAGES_NOTICE_MESSAGE_ID
end
...
---@param messages PiMessage[]|nil
---@return PiMessage[] visible_messages
...
function M.setup_subscriptions(subscribe)
  ...
  if subscribe then
    state.store.subscribe('is_pi_focused', M.on_focus_changed)
    state.store.subscribe('active_session', M.on_session_changed)
  else
    state.store.unsubscribe('is_pi_focused', M.on_focus_changed)
...
local function fetch_session()
  ...
  return require('pi.session').get_messages(session)
end
...
function M._render_full_session_data(session_data, opts)
  ...
  if revert_index then
    local revert_message = {
      info = {
        id = '__pi_revert_message__',
        sessionID = state.active_session.id,
        role = 'system',
      },
      parts = {
        {
          id = '__pi_revert_part__',
          messageID = '__pi_revert_message__',
          sessionID = state.active_session.id,
          type = 'revert-display',
          state = {
  ...
  if opts.restore_model_from_messages then
    require('pi.services.agent_model').initialize_current_model({ restore_from_messages = true })
  end
...
function M.render_from_cache(session_data)
  ...
  if active_session and active_session.id then
    require('pi.ui.question_window').restore_pending_question(active_session.id)
...
function M.render_lines(lines)
  local output = require('pi.ui.output'):new()
  output.lines = lines
