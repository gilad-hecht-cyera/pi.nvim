...
local quick_chat = require('pi.quick_chat')
local history = require('pi.history')
local window_handler = require('pi.commands.handlers.window')
...
local function run_with_opts(prompt, opts)
  return session_runtime.open(opts):and_then(function()
    return require('pi.services.messaging').send_message(prompt, opts)
  end)
end
...
local function run_debug_action(debug_action)
  ...
  local debug_helper = require('pi.ui.debug_helper')
...
function M.actions.select_history()
  require('pi.ui.history_picker').pick()
end
...
function M.actions.references()
  require('pi.ui.reference_picker').pick()
end
...
function M.actions.mention_file()
  local picker = require('pi.ui.file_picker')
...
function M.actions.next_message()
  require('pi.ui.navigation').goto_next_message()
end

function M.actions.prev_message()
  require('pi.ui.navigation').goto_prev_message()
end

function M.actions.next_user_message()
  require('pi.ui.navigation').goto_next_user_message()
end

function M.actions.prev_user_message()
  require('pi.ui.navigation').goto_prev_user_message()
end
...
function M.actions.navigate_to_location(file_path, line, col)
  require('pi.ui.navigation').navigate_to_location(file_path, line, col)
end
...
M.actions.review = Promise.async(function(args)
  ...
  if not require('pi.services.agent_model').initialize_current_model():await() or not state.current_model then
