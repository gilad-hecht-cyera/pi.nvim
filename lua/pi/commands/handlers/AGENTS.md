...
- Prefer `services/*` over direct new `pi.session` / `pi.api` requires in handlers
...
The following handler files still directly require `pi.session` and should be routed via services APIs.

- [ ] `lua/pi/commands/handlers/diff.lua` -> `pi.session`
- [ ] `lua/pi/commands/handlers/session.lua` -> `pi.session`

Sync rule:
- keep this list aligned with `lua/pi/services/AGENTS.md`
...
- Any new direct `require('pi.session')` or `require('pi.api')` in handlers without exception note
