...
local window_actions = require('pi.commands.handlers.window').actions
...
---@param callback fun(state_obj: PiState): any
---@return any
...
local function find_last_user_message(state_obj)
  ...
  local revert_index = revert
    and require('pi.util').find_index_of(messages, function(m)
...
function M.actions.timeline()
  ...
  local timeline_picker = require('pi.ui.timeline_picker')
  timeline_picker.pick(user_messages, function(selected_msg)
    if selected_msg then
      require('pi.ui.navigation').goto_message_by_id(selected_msg.info.id)
    end
  end)
end
