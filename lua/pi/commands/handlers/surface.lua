...
---@return table<string, PiUICommand>
local function get_command_definitions()
  return require('pi.commands').get_commands()
end
...
function M.actions.with_header(lines, show_welcome)
  ...
  if show_welcome then
    table.insert(
      msg,
      'Welcome to Pi.nvim! This plugin allows you to interact with AI models directly from Neovim.'
    )
    table.insert(msg, '')
  end
...
function M.actions.help()
  ...
  local msg = M.actions.with_header({
    '### Available Commands',
    '',
    'Use `:Pi <subcommand>` to run commands. Examples:',
    '',
    '- `:Pi open input` - Open the input window',
    '- `:Pi session new` - Create a new session',
    '- `:Pi diff open` - Open diff view',
    '',
    '### Subcommands',
    '',
    '| Command      | Description |',
...
M.actions.commands_list = Promise.async(function()
  ...
  if not commands then
    vim.notify('No user commands found. Please check your pi config file.', vim.log.levels.WARN)
    return
  end
...
M.actions.mcp = Promise.async(function()
  local mcp_picker = require('pi.ui.mcp_picker')
  mcp_picker.pick()
end)

M.actions.skills = Promise.async(function()
  local skill_picker = require('pi.ui.skill_picker')
  skill_picker.pick()
end)
