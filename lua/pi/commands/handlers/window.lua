...
local function build_toggle_open_context(restore_hidden)
  ...
  local focus = config.ui.input.auto_hide and 'input' or state.last_focused_pi_window or 'input'
...
function M.actions.toggle_focus(new_session)
  if not ui.is_pi_focused() then
    local focus = state.last_focused_pi_window or 'input' ---@cast focus 'input' | 'output'
...
M.command_defs = {
  ...
  close = {
    desc = 'Close pi windows',
    execute = M.actions.close,
  },
  hide = {
    desc = 'Hide pi windows (preserve buffers for fast restore)',
  ...
  toggle = {
    desc = 'Toggle pi windows',
    execute = M.actions.toggle,
  },
  toggle_focus = {
    desc = 'Toggle focus between pi and code',
