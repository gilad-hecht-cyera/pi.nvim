...
local ChatContext = require('pi.context.chat_context')
local QuickChatContext = require('pi.context.quick_chat_context')
local BaseContext = require('pi.context.base_context')

local M = {}

---@type table<PiToggleableContextKey, boolean>
...
---@param context_key PiToggleableContextKey
---@return table
...
---@param context_config? PiContextConfig Optional context config
---@param opts? { range?: { start: integer, stop: integer } }
---@return table result { parts: PiMessagePart[] }
...
---@param context_config? PiContextConfig Optional context config
---@param opts? { range?: { start: integer, stop: integer } }
---@return table result { text: string, parts: PiMessagePart[] }
...
--- into the pi input buffer, in the form:
---
---   **`path/to/file`**
---
---   ```<filetype>
...
---@param prompt string
---@param opts? PiContextConfig|nil
---@return PiMessagePart[]
...
--- Extracts context from an PiMessage (with parts)
---@param message { parts: PiMessagePart[] }
---@return { prompt: string|nil, selected_text: string|nil, current_file: string|nil, mentioned_files: string[]|nil}
function M.extract_from_pi_message(message)
...
function M.setup()
  ...
  state.store.subscribe({ 'current_code_buf', 'current_context_config', 'is_pi_focused' }, function()
    debounced_load()
  end)

  local augroup = vim.api.nvim_create_augroup('PiContext', { clear = true })
