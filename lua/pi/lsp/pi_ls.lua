---In-process LSP server for pi completion
...
handlers[ms.workspace_executeCommand] = function(params, callback)
  if params.command == 'pi.completion_done' then
    ...
    if item then
      require('pi.ui.completion').on_completion_done(item)
    end
    callback(nil, nil)
  else
    callback({
...
function M.supports_kind_icons()
  return require('pi.ui.completion').supports_kind_icons()
end

---Convert pi CompletionItem to LSP CompletionItem
...
local function to_lsp_item(item, index, params, word)
  local source = require('pi.ui.completion').get_source_by_name(item.source_name)
...
function M.start(bufnr)
  ...
  local augroup = vim.api.nvim_create_augroup('PiLspCompletion_' .. bufnr, { clear = true })
  ...
  vim.api.nvim_create_autocmd('CompleteDonePre', {
    ...
    callback = function()
      ...
      if completed_item and completed_item.user_data then
        ...
        local item = data and data._pi_item
