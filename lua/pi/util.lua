...
function M.safe_pcall(fn, ...)
  ...
  if not ok then
    vim.schedule(function()
      vim.notify('[pi.nvim] Unexpected error: ' .. vim.inspect(err))
    end)
  end
end
...
function M.parse_run_args(args)
  ...
  if prompt:find('#') then
    ...
    opts.context = vim.tbl_deep_extend('force', opts.context or {}, quick_context) --[[@as PiContextConfig]]
  end
...
--- @return string prompt, PiContextConfig config
