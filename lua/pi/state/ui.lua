...
---@class PiUiStateMutations
local M = {}

local _state = store.state()

---@param windows PiWindowState|nil
...
function M.set_panel_focused(is_focused)
  return store.set('is_pi_focused', is_focused)
end

---@param win_type 'input'|'output'|nil
function M.set_last_focused_window(win_type)
  return store.set('last_focused_pi_window', win_type)
end
...
function M.set_last_code_window(win_id)
  return store.set('last_code_win_before_pi', win_id)
end
...
function M.set_code_context(win_id, bufnr)
  store.batch(function()
    store.set('last_code_win_before_pi', win_id)
...
---@param hidden PiHiddenBuffers|nil
---@return PiHiddenBuffers|nil
...
---@return {status: 'closed'|'hidden'|'visible', position: string, windows: PiWindowState|nil, cursor_positions: {input: integer[]|nil, output: integer[]|nil}}
