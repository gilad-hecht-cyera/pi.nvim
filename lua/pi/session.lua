...
---Get the current Pi project ID
---@return string|nil
M.project_id = Promise.async(function()
  local project = config_file.get_pi_project():await()
  if not project then
    vim.notify('No Pi project found in the current directory', vim.log.levels.ERROR)
    return nil
  end
  return project.id
end)

---Get the base storage path for Pi
---@return string
function M.get_storage_path()
  local home = vim.uv.os_homedir()
  return home .. '/.local/share/pi/storage'
end
...
M.get_workspace_session_path = Promise.async(function(project_id)
  ...
  local home = vim.uv.os_homedir()
  return home .. '/.local/share/pi/storage/session/' .. project_id
end)

function M.get_cache_path(session_id)
  local cache_base = vim.fn.stdpath('cache') .. '/pi/session/'
