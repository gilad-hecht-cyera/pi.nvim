...
---@type table<string, PiQuickChatRunningSession>
...
---@param message PiMessage Message object
...
---@param session_info table Session tracking info
---@param messages PiMessage[] Session messages
...
local function process_response(session_info, messages, range)
  ...
  ---@cast response_message PiMessage
...
local on_done = Promise.async(function(active_session)
  ...
  local messages = session.get_messages(active_session):await() --[[@as PiMessage[] ]]
...
---@return PiContextConfig context_opts
...
---@param context_config PiContextConfig Context configuration
...
---@param options {context_config?:PiContextConfig, model?: string, agent?: string}|nil Optional configuration for context and behavior
...
function M.setup()
  local augroup = vim.api.nvim_create_augroup('PiQuickChat', { clear = true })
