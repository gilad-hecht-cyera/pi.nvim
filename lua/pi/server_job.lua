...
function M.unregister_port_usage(port)
  port_mapping.unregister(port, state.pi_server)
end
...
local function try_custom_server(base_url, timeout)
  ...
  return pi_server.health_check(health_url, timeout * 1000):and_then(function(healthy)
    if healthy then
...
--- Make an HTTP API call to the pi server.
--- @generic T
...
--- Make a streaming HTTP API call to the pi server.
--- @param url string The API endpoint URL
--- @param method string|nil HTTP method (default: 'GET')
--- @param body table|nil|boolean Request body (will be JSON encoded)
...
local function pi_rpc_server()
  ...
  function server:is_running()
    return require('pi.pi_process').get():is_running()
  end
  ...
  function server:shutdown()
    require('pi.pi_process').get():stop()
    self.shutdown_promise:resolve(true)
    return self.shutdown_promise
  end
  require('pi.rpc_client').get().process:start():and_then(function()
...
--- Ensure the pi server is running, starting it if necessary.
--- @return Promise<PiServer>
function M.ensure_server()
  if config.backend == 'pi' then
    if state.pi_server and state.pi_server.mode == 'pi-rpc' then
      if require('pi.pi_process').get():is_running() then
        return Promise.new():resolve(state.pi_server)
      end
      return require('pi.rpc_client').get().process:start():and_then(function()
        return state.pi_server
      end)
    end
  ...
  if state.pi_server and state.pi_server:is_running() then
    return state.pi_server:check_health():and_then(function(healthy)
      if healthy then
        return state.pi_server
      end
...
local function spawn_and_retry(base_url, custom_port, custom_url, promise, timeout)
  ...
  retry_connect(base_url, timeout, 3, function(url)
    ...
    state.jobs.set_server(pi_server.from_custom(url, custom_port, 'custom'))
    promise:resolve(state.pi_server)
  end, function(_err)
...
function M.try_connect_to_custom_server(base_url, timeout, promise, custom_port, custom_url)
  try_custom_server(base_url, timeout)
    :and_then(function(url)
      ...
      state.jobs.set_server(pi_server.from_custom(url, custom_port, mode))
      log.notify(
    ...
    :catch(function(err)
      ...
      elseif not config.server.auto_kill then
        ...
        retry_connect(base_url, timeout, 5, function(url)
          ...
          state.jobs.set_server(pi_server.from_custom(url, custom_port, 'attach'))
          log.notify(
...
--- @param promise Promise<PiServer>
...
function M.spawn_local_server(promise, port, hostname)
  state.jobs.set_server(pi_server.new())
  ...
  state.pi_server:spawn(spawn_opts)
end
