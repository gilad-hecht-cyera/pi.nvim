...
local ThrottlingEmitter = require('pi.throttling_emitter')
...
--- @field properties {part: PiMessagePart}
...
--- @field properties PiQuestionRequest
...
--- @field state_server_listener function|nil Listener for state.pi_server updates
...
--- @param event_name PiEventName The event name to listen for
...
--- @overload fun(self: EventManager, event_name: "custom.emit_events.started", callback: fun(): nil)
--- @overload fun(self: EventManager, event_name: "custom.emit_events.finished", callback: fun(): nil)
--- @param event_name PiEventName The event name
...
function EventManager:start()
  ...
  if self.state_server_listener then
    state.store.unsubscribe('pi_server', self.state_server_listener)
  end
  ...
  state.store.subscribe('pi_server', self.state_server_listener)
  ...
  self.state_cwd_listener = function(key, new_cwd, old_cwd)
    if new_cwd ~= old_cwd and state.pi_server and state.pi_server.url then
      log.debug('Directory changed from %s to %s, re-subscribing to server events', old_cwd, new_cwd)
      self:_subscribe_to_server_events(state.pi_server)
    end
  end
