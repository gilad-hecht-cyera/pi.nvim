...
--- @field url string|nil The URL the pi server is listening on
--- @field server_pid number|nil The PID of the pi server process (local servers only)

--- @return string
local function file_path()
  return vim.fn.stdpath('data') .. '/pi_port_mappings.json'
end
...
local function kill_orphaned_server(port, server_pid)
  ...
  PiServer.request_graceful_shutdown(base_url)

  if server_pid then
    PiServer.kill_pid(server_pid)
  else
...
--- @param server PiServer instance (state.pi_server)
